...
bool Houses::loadHousesXML(const std::string& filename)
{
	...
	if (!result) {
		printXMLError("Houses::loadHousesXML", filename, result);
		return false;
	}
	...
	for (auto houseNode : doc.child("houses").children()) {
		...
		if (!house) {
			console::reportError("Houses::loadHousesXML", fmt::format("Unknown house with id: {:d}!", houseId));
			return false;
		}
		...
		if (entryPos.x == 0 && entryPos.y == 0 && entryPos.z == 0) {
			console::reportWarning("Houses::loadHousesXML", fmt::format("No exit defined for house \"{:s}\" (id: {:d})!", house->getName(), houseId));
		}
