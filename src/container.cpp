...
Container::Container(Tile* tile) : Container(ITEM_BROWSEFIELD, 30, false, true)
{
	...
	if (itemVector) {
		for (Item* item : *itemVector) {
			if ((item->getContainer() || item->hasForceSerialize() || item->hasProperty(CONST_PROP_MOVEABLE)) && !item->hasAttribute(ITEM_ATTRIBUTE_UNIQUEID)) {
		...
		uint32_t flags, Creature* actor/* = nullptr*/) const
{
	...
	const Container* container = thing.getContainer();
	if ((item->isStoreItem() || container && container->hasStoreItems()) && !dynamic_cast<const DepotChest*>(this->getTopParent())) {
