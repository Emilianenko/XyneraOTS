...
class Game
{
	public:
		...
		void loadLatestRewardId();
		void saveLatestRewardId();
		...
		void playerEquipItem(uint32_t playerId, uint16_t spriteId, bool hasTier = false, uint8_t tier = 0);
		...
		void playerBrowseMarket(uint32_t playerId, uint16_t spriteId, uint8_t tier);
		...
		bool combatChangeHealth(Creature* attacker, Creature* target, CombatDamage& damage, bool isReflect = false);
		bool combatChangeMana(Creature* attacker, Creature* target, CombatDamage& damage, bool isReflect = false);
