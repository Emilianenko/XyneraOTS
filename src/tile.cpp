...
{
	if (item->hasProperty(CONST_PROP_MOVEABLE) || item->getContainer() || item->hasForceSerialize()) {
...
{
	if (newItem->hasProperty(CONST_PROP_MOVEABLE) || newItem->getContainer() || newItem->hasForceSerialize()) {
		auto it = g_game.browseFields.find(this);
		if (it != g_game.browseFields.end()) {
	...
	} else if (oldItem->hasProperty(CONST_PROP_MOVEABLE) || oldItem->getContainer() || oldItem->hasForceSerialize()) {
		auto it = g_game.browseFields.find(this);
		if (it != g_game.browseFields.end()) {
...
ReturnValue Tile::queryAdd(int32_t, const Thing& thing, uint32_t, uint32_t flags, Creature*) const
{
	if (const Creature* creature = thing.getCreature()) {
		...
		if (hasBitSet(FLAG_CHECKMOVEABLE, flags) && hasFlag(TILESTATE_BLOCKPATH)) {
		...
		if (const Monster* monster = creature->getMonster()) {
			if (!monster->isFamiliar() && hasFlag(TILESTATE_PROTECTIONZONE)) {
				return RETURNVALUE_NOTPOSSIBLE;
			}
