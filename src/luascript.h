...
class LuaScriptInterface
{
	...
	protected:
		...
		static int32_t runningEventId; // = EVENT_ID_USER; / previously not static

		//script file cache
		static std::map<int32_t, std::string> cacheFiles; // previously not static
	...
	private:
		...
		static int luaGameGetExperienceStages(lua_State* L);
		...
		static int luaGameSetStoreOutfit(lua_State* L);
		static int luaGameSetStoreMount(lua_State* L);
		...
		static int luaCreatureGetDisplayMode(lua_State* L);
		static int luaCreatureSetDisplayMode(lua_State* L);
		...
		static int luaPlayerGetAccountResource(lua_State* L);
		static int luaPlayerSetAccountResource(lua_State* L);
		static int luaPlayerAddAccountResource(lua_State* L);
		static int luaPlayerSaveAccountResource(lua_State* L);

		static int luaPlayerGetCapacity(lua_State* L);
		static int luaPlayerSetCapacity(lua_State* L);

		static int luaPlayerGetBaseCapacity(lua_State* L);
		...
		static int luaMonsterIsFamiliar(lua_State* L);
		static int luaMonsterSetFamiliar(lua_State* L);
		...
		static int luaGuildGetLeaderRankLevel(lua_State* L);
		...
		static int luaMonsterTypeGetReflectList(lua_State* L);
